...
import { Search, RefreshCcw, Loader2, Info, CheckCircle } from "lucide-react";
...
export function OrdersTable({ initialOrders }: { initialOrders: Order[] }) {
  ...
  const handleManualApprove = async (id: string) => {
    if (!confirm("Deseja marcar este pedido como entregue manualmente?")) return;
    
    try {
      const res = await fetch(`/api/admin/orders/${id}/manual-approve`, { method: "POST" });
      if (res.ok) {
      ...
      if (!order.panelOrderId) {
        ...
        if (smm) {
           ...
           const s = smm.status?.toLowerCase() || "";
            ...
            if (order.panelOrderId === "MANUAL") finalStatus = "Concluído (Manual)";
                    ...
                    <div className="flex flex-col items-center justify-center gap-1.5 mt-1.5">
                      <div className="flex items-center justify-center gap-1.5">
                        <span className={`inline-flex items-center px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-wider border ${
                          order.finalStatus.includes('Concluído') ? 'bg-green-500/10 text-green-500 border-green-500/20' :
                        ...
                        {order.finalStatus === 'Erro SMM' && order.smmError && (
                      ...
                      {order.finalStatus === 'Erro SMM' && (
                        <div className="flex items-center gap-1.5">
                          <button 
                            ...
                            className="p-1.5 rounded-lg bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 transition-all group/btn"
                          ...
                          <button 
                            ...
                            className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all"
                            title="Validar entrega manualmente"
                          >
